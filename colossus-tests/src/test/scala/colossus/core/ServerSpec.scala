    ...
    "shutdown when it cannot bind to a port when a duration is supplied" in {
      ...
      "shutting down a system kills client connections"  in {
        ...
        withServer(server) {
          ...
          val c = TestClient(cio, TEST_PORT, connectionAttempts = PollingDuration.none)
      ...
      "open up spot when connection closes" in {
        ...
        withIOSystemAndServer(simpleEchoDelegator, Some(settings)) {(io, server) => {
          val c1 = TestClient(server.system, TEST_PORT)
          expectConnections(server, 1)
          val c2 = TestClient(server.system, TEST_PORT, waitForConnected = false, connectionAttempts = PollingDuration.none)
          ...
          val c3 = TestClient(server.system, TEST_PORT, waitForConnected = true, connectionAttempts = PollingDuration.none)
      ...
      "close connection when worker rejects" in {
        ...
        withIOSystemAndServer((s,w) => new AngryDelegator(s,w)) {(io, server) => {
          val c1 = TestClient(server.system, TEST_PORT, connectionAttempts = PollingDuration.none, waitForConnected = false)
          intercept[service.ServiceClientException] {
      ...
      "times out idle client connection" in {
        withIOSystem { implicit io =>
          ...
          probe watch server.server
          waitForServer(server)
          val c = TestClient(server.system, TEST_PORT, connectionAttempts = PollingDuration.none)
          expectConnections(server, 1)
          Thread.sleep(1000)
          TestUtil.expectServerConnections(server, 0)
      ...
      "switch to high water timeout when connection count passes the high water mark" in {
        ...
        withIOSystem { implicit io =>
          ...
          val server = Server(config)
          withServer(server) {
            val idleConnections = for{i <- 1 to 9} yield TestClient(server.system, TEST_PORT, connectionAttempts = PollingDuration.none)
